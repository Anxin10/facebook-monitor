...
def format_datetime(
    dt: Optional[datetime], tz_name: str = "Asia/Taipei"
) -> Optional[str]:
    """格式化日期時間
...
def retry_with_backoff(
    func, max_attempts: int = 5, base_delay: float = 30.0, backoff_factor: float = 2.0
):
    """重試裝飾器，帶有指數退避
    ...
    def wrapper(*args, **kwargs):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logging.warning(
                    f"嘗試 {attempt + 1}/{max_attempts} 失敗，{delay:.1f}秒後重試：{e}"
                )
                time.sleep(delay)
