...
class GraphClient:
    ...
    def __init__(
        ...
        self.session.headers.update({"User-Agent": "facebook-monitor/0.2"})
...
class PostFetcher:
    ...
    def __init__(self, config: Dict[str, Any], logger: logging.Logger):
        ...
        self.api_version = self.posts_config.get(
            "api_version", GraphClient.DEFAULT_VERSION
        )
    ...
    def fetch_page_posts(self, page_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            baseline_ready = (
                check_status.get("baseline_ready", False) if check_status else False
            )
            last_success_at = (
                check_status.get("last_success_at") if check_status else None
            )
            ...
            update_check_status(
                self.db_path, page_id, "post", "failed", "exception", str(e)[:200]
            )
            return []
    ...
    def _fetch_posts(
        ...
        try:
            ...
            return (
                posts,
                True,
                {"code": str(e.response.status_code), "message": error_msg},
            )
    ...
    def _save_new_posts(
        ...
        for post in new_posts:
            ...
            if enabled_channels and (
                baseline_ready or self.notify_existing_on_first_run
            ):
