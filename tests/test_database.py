...
from database import (
    init_database, add_item, item_exists, get_items_by_page,
    update_check_status, get_check_status, set_baseline_ready,
    add_notifications, get_pending_notifications,
    update_notification_status, save_batch_with_transaction
)
...
class TestDatabase(unittest.TestCase):
    ...
    def test_item_exists(self):
        ...
        self.assertFalse(item_exists(
            self.db_path, 'test_page', 'post', 'post_2'
        ))
    ...
    def test_author_filter(self):
        ...
        add_item(
            self.db_path, 'test_page', 'post', 'post_1',
            author_id='page_id', published_at=datetime.now(timezone.utc)
        )
        add_item(
            self.db_path, 'test_page', 'post', 'post_2',
            author_id='other_page', published_at=datetime.now(timezone.utc)
        )
    ...
    def test_pagination_incomplete(self):
        ...
        update_check_status(
            self.db_path, 'test_page', 'post',
            'success', None, None, pagination_incomplete=True
        )
    ...
    def test_three_page_cursor(self):
        """測試三頁游標分頁"""
        # 模擬三頁貼文
        for page in range(3):
            for i in range(3):
                ...
                add_item(
                    self.db_path, 'test_page', 'post', content_id,
                    author_id='page_id',
                    published_at=datetime.now(timezone.utc) - timedelta(hours=page*10+i)
                )
        ...
        # 檢查排序（最新的在前）
        self.assertEqual(items[0]['content_id'], 'page2_post2')
    ...
    def test_notification_retry(self):
        ...
        update_notification_status(
            self.db_path, notification_id, 'failed', 'Test error'
        )
        
        # 檢查狀態回到 pending，且有 next_retry_at
        pending = get_pending_notifications(self.db_path)
        self.assertEqual(len(pending), 1)
