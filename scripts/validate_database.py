...
class DatabaseValidator:
    ...
    def test_check_operations(self):
        ...
        try:
            # 更新檢查狀態
            update_check_status(
                self.db_path, self.test_page_id, "post", "success", None, None
            )
    ...
    def test_notification_operations(self):
        ...
        try:
            ...
            result = add_notification(
                self.db_path, self.test_page_id, "post", "test_post_1", "line"
            )
            ...
            # 更新通知狀態為已發送
            if pending:
                update_notification_status(self.db_path, pending[0]["id"], "sent")
                self.log("通知狀態更新為 'sent'", "INFO")

            # 測試失敗重試
            update_notification_status(
                self.db_path, pending[0]["id"], "failed", "網路錯誤"
            )
