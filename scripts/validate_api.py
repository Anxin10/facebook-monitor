...
class APIValidator:
    ...
    def validate_token(self):
        ...
        try:
            ...
            if 'id' in data:
                ...
                self.log(f"Token 對應使用者：{data.get('name')} ({data.get('id')})", "INFO")
            else:
            ...
            if 'data' in data:
                ...
                if post_count > 0:
                    ...
                    for i, post in enumerate(posts[:3], 1):
                        ...
                        created = post.get('created_time', 'N/A')[:10] if post.get('created_time') else 'N/A'
                        self.log(f"  {i}. {post_id} [{post_type}] {created}", "INFO")
                    
                    # /posts 端點保證是粉專發布，不需要作者篩選
                    self.add_result('作者保證', True, "/posts 端點直接返回粉專發布的貼文")
                else:
                ...
                if paging.get('next'):
                    ...
                    if cursors.get('after'):
                        self.add_result('Cursor Pagination', True, 
                                       f"支援 cursors.after: {cursors['after'][:30]}...")
                    else:
                        self.add_result('Cursor Pagination', False, 
                                       "paging 無 cursors.after")
                else:
                    self.log("無更多分頁", "INFO")
    ...
    def run_all_tests(self):
        ...
        print(f"Token: {self.access_token[:20]}..." if self.access_token else "Token: 未設定")
        print("#"*60 + "\n")
